...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_remaining FROM projects
INNER JOIN pledges ON projects.id = pledges.project_id
GROUP BY projects.title HAVING amount_remaining >= 0;"
end
...
JOIN pledges ON users.id = pledges.user_id
GROUP BY users_id
ORDER BY (SUM(pledges.amount),pledges.users.name;
"
end
